function displayPopAvgRightLongPCSorted(varargin)
% displayPopAvgRightLongPCSorted
% adr
% ea lab
...
ROYGBIV = imread(fileDirs.phiMapColorLUT);

popAvgPlotObj = plotBinnedPopAvgs('filename','displayPopAvgRightLongPCSorted','autoSubplotSpacing',false,'axesSpacing',0.01,...
    'LineColor',ROYGBIV,'Marker','.','MarkerSize',3,'FaceAlpha',0.1,'FontSize',6,'paperPosition',[0 0.5 11 3],'XLIM',[-4.666 5.333],'YLIM',[-17 730],...
    'zeroLineYMax',730,'zeroLineWidth',0.3,'zeroLineColor',[0 0 0],'zeroLineStyle','--',...
    ...
    'YLabelPosition',[-0.25,0.1],'sortName','\bf \phi ','sortNameLabelPosition',[-5.667,720.09],'sortNameEqualPosition',[-3.467,720.085],'sortNameValuePosition',[-1,720.085],...
...
if saveCSV
    popAvgPlotObj.toCSV('X',STACAT,'sortValue',lon,'extraBinCriteria',isLeftSaccade,'binCenters',binCenters,'time',tauPCA,...
        'ID',options.ID,'figureName','Supplementary Figure 3.csv');
end
